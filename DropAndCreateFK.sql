...
SET NOCOUNT ON  
declare @tab table (commande   varchar(max), chaine   varchar(max)) 
...
SET @tableName = 'tblarchives%'  -- 'tblArchivesSondagesPlages' --
SET @schemaName = 'dbo'  
...
BEGIN   
   ...
   BEGIN  
       ...
       SET @cmd = '   IF OBJECT_ID (N'''+ @FK_NAME +''', N''F'') IS NOT NULL  BEGIN  ALTER TABLE [' + @FKTABLE_OWNER + '].[' + @FKTABLE_NAME   + ']  DROP CONSTRAINT [' + @FK_NAME + '] select ''OK'' END ELSE BEGIN SELECT ''ERROR'' END '     
      ...
      PRINT @cmd  
       ...
       ORDER BY Fk_Cl.constraint_column_id  
		...
		insert into @tab (commande, chaine ) 
		values (@operation ,  @cmd) 
       ...
       SET @cmd = 'IF OBJECT_ID (N'''+ @FK_NAME +''', N''F'') IS NULL BEGIN ALTER TABLE [' + @FKTABLE_OWNER + '].[' + @FKTABLE_NAME + ']  WITH ' +   
           CASE @FK_DISABLED   
               WHEN 0 THEN ' CHECK '  
               WHEN 1 THEN ' NOCHECK '  
           ...
           CASE @FK_NOT_FOR_REPLICATION  
               WHEN 0 THEN ''  
               WHEN 1 THEN ' NOT FOR REPLICATION select ''OK'' END ELSE BEGIN SELECT ''ERROR'' END '       
           END  

      set  @cmd  = @cmd + ' select ''OK'' END ELSE BEGIN SELECT ''ERROR'' END '  
